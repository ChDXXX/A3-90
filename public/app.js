(() => {
  ...
  window.tokenA2 = null;          // Cognito Id/Access Token for /api/cloud/*
  ...
  function pickCognitoToken(payload) {
    ...
    return (
      ...
      a.accessToken || a.AccessToken || a.access_token ||
      ar.AccessToken || ar.accessToken || ar.access_token ||
      null
    );
  }
  ...
  async function cg_login() {
    ...
    $('cg_whoami').textContent = 'Cognito login OK';
  }
  ...
  async function s3_upload() {
    ...
    $('s3_status').textContent = putResp.ok ? 'Uploaded to S3 ✅' : 'Upload failed ❌';
  }
  ...
  async function login() {
    ...
    tokenA1 = data.token;
    $('whoami').textContent = 'Logged in as ' + (data.user?.username || username);
    loadFiles(1);
  }
  ...
  async function loadFiles(page = 1) {
    if (!tokenA1) return;
    const q = $('q').value; const sort = $('sort').value; const order = $('order').value;
  ...
  async function transcodeSync(id) {
    ...
    const data = await resp.json(); $(`out_${id}`).textContent = JSON.stringify(data);
  }
  ...
  async function transcodeAsync(id) {
    ...
    const data = await resp.json(); $(`out_${id}`).textContent = 'Job ' + data.jobId + ' ' + data.status;
  }
  ...
  async function yt(q, vid) {
    ...
    const data = await resp.json(); if (!resp.ok) { alert(data.error || 'YouTube error'); return; }
  ...
  async function tmdb(q, vid) {
    ...
    const data = await resp.json(); if (!resp.ok) { alert(data.error || 'TMDB error'); return; }
  ...
  async function pixabay(q, vid) {
    ...
    const data = await resp.json(); if (!resp.ok) { alert(data.error || 'Pixabay error'); return; }
